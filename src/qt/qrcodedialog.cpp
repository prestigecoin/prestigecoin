...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BOX")
    updateDisplayUnit();
}
...
{
    QString ret = QString("boxxycoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BOX unit input in lnReqAmount, we generate the URI with BOX as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BOX, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
